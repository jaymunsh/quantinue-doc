...
- 디자인: 좌측 고정 **파이프라인 척추(01→11) 목차** = 시그니처 · 모노(JetBrains Mono) 계기판 타이포 + 본문 Apple SD Gothic Neo→Noto Sans KR(CDN) · 틸 브랜드색 + 의미 상태색 분리 · **다크모드**(기본 라이트, localStorage 기억) · 모바일 **햄버거 서랍** 목차.
- 섹션 15개 뼈대(플레이스홀더 `◍ 채우는 중`): 현재상태·2차확정·용어·전체흐름·스케줄·데이터모델·파이프라인01~11·방어선·계좌·화면·개발계획·AI제안·결정로그·보안·변경이력.
- 링크: 1차 실제 화면 = `quantinue-mvp1-showcase.html`(정적 시연, 사용자 제작), 1차 설계 = `-mvp1.html` 동결본.
- 기간 표기 '26.06.29–07.29. 다음: 섹션 내용 채우기(Phase 1·2 산출물 이관), ① 2차 확정사항부터.

### 07-18 — 2차(MVP-2) 기획: Phase 0 감사 + Phase 1 결정 완료 (`docs/mvp2-planning/`)

- **Phase 0** = `phase0-asbuilt-audit.md`: as-built 감사 (Explore 4병렬). 핵심: 깔때기 코드 50→20→20(원설계 2,000→50의 임의 축소), 스케줄 미배선, cycle_ts 멱등 파손(now() 분내림), 방어선 게이트 전부 하드코딩 껍데기, gpt-4o-mini 단일, 회고 미완. 반대로 계보 10컬럼·운영 4테이블·주문 안전장치는 문서보다 선행.
- **Phase 1** = `phase1-decisions.md`: 결정 8라운드 — R1 2차=AI 펀드 서비스(관리자 ERP+유저 read-only 포털, 성향 공격형/안전형 2종, 리서치 공용·계좌 구독) / R2 앱 내장 스케줄러+슬롯 멱등+역할별 실행 창 / R3 방어선 3종+숫자 일괄 확정(⚙️조정 가능 원칙) / R4 LLM 3단(mock·qwen3.6-35b·gpt-4o-mini) / R5 신호 품질 7건(3건 실무 개정) / R6 회고 자동화+메모리+비용 가드 / R7 실무 보강 6건(보유 재평가·청산 3층, 서킷브레이커, 텔레그램, NYSE 캘린더, 계좌 대사, SPY 벤치마크) / R8 깔때기 원설계 복원(16,000→2,000 주1회→50 매일→20+보유 LLM).
- 사용자 위임: 미흡한 리서치는 Claude 실무 관점으로 교체 가능 (메모리 저장됨). 다음 = Phase 2(writing-plans 개발 계획서) → Phase 3(frontend-design v4.0 HTML).
