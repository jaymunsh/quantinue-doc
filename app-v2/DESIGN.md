...
- `stage-record`: ordered two-digit marker, role and summary, state badge, attempt/timing metadata, checkpoint state, and associated evidence references. The marker, left rail, and text badge share one semantic state while the bordered surface keeps each stage scannable as a distinct ledger record.
- `event-record`: a completed defensive close with explicit reason, ticker/quantity, account, fill price, and timestamp. It uses the warning state without implying failure; empty state means no persisted sell fill for the selected slot.
