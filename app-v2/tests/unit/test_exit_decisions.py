...
def test_the_stop_being_touched_closes_the_position() -> None:
    # Given / When
    decision = decide_bracket(_position(), Decimal("84.00"))

    # Then
    assert decision is not None
    assert decision.reason is ExitReason.STOP
    assert decision.reference_price == Decimal("85.00")


def test_reaching_the_target_closes_the_position() -> None:
    # Given / When
    decision = decide_bracket(_position(), Decimal("121.00"))

    # Then
    assert decision is not None
    assert decision.reason is ExitReason.TAKE_PROFIT
    assert decision.reference_price == Decimal("120.00")


def test_price_between_the_bracket_legs_keeps_the_position_open() -> None:
    # Given / When / Then
    assert decide_bracket(_position(), Decimal("101.00")) is None
...
def test_intraday_bracket_and_daily_hard_event_keep_separate_reasons() -> None:
    # Given / When
    bracket = decide_bracket(_position(), Decimal("84.00"))
    daily = _decide(hard_event=True)

    # Then
    assert bracket is not None
    assert bracket.reason is ExitReason.STOP
    assert daily is ExitReason.THESIS_BREAK
...
@pytest.mark.parametrize(
    ("reason", "current_price", "expected"),
    [
        (ExitReason.STOP, Decimal("84.00"), "85.00"),
        (ExitReason.TAKE_PROFIT, Decimal("121.00"), "120.00"),
    ],
)
def test_a_triggered_leg_fills_at_its_own_price_not_the_last_trade(
    reason: ExitReason, current_price: Decimal, expected: str
) -> None:
    """대기 주문은 자기 가격에 체결된다 — 종가로 찍으면 손익이 왜곡된다."""
    # Given/When
    decision = decide_bracket(_position(), current_price)
...
def test_intraday_bracket_does_not_share_the_daily_sell_reason() -> None:
    # Given / When
    bracket = decide_bracket(_position(), Decimal("84.00"))
    daily = _decide(sell_signals=frozenset({"aggressive"}))

    # Then
    assert bracket is not None
    assert bracket.reason is ExitReason.STOP
    assert daily is ExitReason.THESIS_SOFT
